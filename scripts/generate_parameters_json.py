...
Usage:
    python scripts/generate_parameters_json.py            # write schemas/guardrail_parameters.json
    python scripts/generate_parameters_json.py --check    # exit non-zero if the committed file is stale
"""
...
def main() -> int:
    """Generate or check the committed guardrail parameters JSON."""
    ...
    if args.check:
        if not args.out.exists() or args.out.read_text(encoding="utf-8") != content:
            print(
                f"{args.out} is out of date. Run `python scripts/generate_parameters_json.py` and commit the result.",
                file=sys.stderr,
            )
            return 1
        return 0
