...
class LlamafileProvider(Provider[AnyDict, AnyDict]):
    ...
    process can grab it before the llamafile binary binds — a TOCTOU race.
    Retrying with a fresh port closes the window. When the user pins the
    port (``self.port is not None``) we don't retry: a bind failure is a
    config problem to surface, not a race.
    """
    ...
    def _spawn_subprocess(self, binary: Path, model_id: str) -> None:
        """Pick a port, spawn the llamafile subprocess, and update provider state."""
