...
class OffTopic(HuggingFace):
    ...
    def _load_model(self) -> None:
        ...
        elif self.model_id == "mozilla-ai/jina-embeddings-v2-small-en-off-topic":
    ...
    def _pre_processing(self, input_text: str, comparison_text: str = "") -> Tuple[torch.Tensor, ...]:
        if self.model_id == "mozilla-ai/stsb-roberta-base-off-topic":
            ...
            inputs1 = self.tokenizer(input_text, return_tensors="pt", truncation=True, padding="max_length", max_length=1024)
            inputs2 = self.tokenizer(comparison_text, return_tensors="pt", truncation=True, padding="max_length", max_length=1024)
    ...
    def _inference(self, model_inputs: Tuple[torch.Tensor, ...]) -> Any:
        if self.model_id == "mozilla-ai/stsb-roberta-base-off-topic":
            ...
            assert input_ids2 is not None and attention_mask2 is not None, "Comparison text inputs must be provided for the shared base model."
            with torch.no_grad():
                return self.model(input_ids1=input_ids1, attention_mask1=attention_mask1,
    ...
    def _post_processing(self, model_outputs: Any) -> GuardrailOutput:
        ...
        probs_dict = {
            "on-topic": explanatory_probs[0],
            "off-topic": explanatory_probs[1]
        }
