...
class OffTopicJina(HuggingFace):
    ...
    def _pre_processing(self, input_text: str, comparison_text: str = "") -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        warnings.warn("Truncating input text to a max length of 1024 tokens.")
    ...
    def _inference(self, model_inputs: tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor,]) -> Any:
