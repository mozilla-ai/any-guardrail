...
from any_guardrail import instantiate_guardrail
...
def test_integration_instantiate_and_classify():
    ...
    with patch("any_guardrail.instantiate_guardrail.instantiate_guardrail") as mock_instantiate:
        ...
        classes = instantiate_guardrail.list_guardrail_classes()
        assert len(classes) > 0
        
        # Test instantiation of a guardrail
        guardrail = instantiate_guardrail.instantiate_guardrail("jasper", modelpath="dummy-path")
...
def test_integration_error_propagation():
    ...
    with patch("any_guardrail.instantiate_guardrail.instantiate_guardrail", 
               side_effect=ValueError("Guardrail not found")):
        with pytest.raises(ValueError, match="Guardrail not found"):
            instantiate_guardrail.instantiate_guardrail("nonexistent", modelpath="dummy-path")
...
def test_integration_guardrail_interface_consistency():
    ...
    with patch("any_guardrail.instantiate_guardrail.instantiate_guardrail") as mock_instantiate:
        ...
        guardrail = instantiate_guardrail.instantiate_guardrail(guardrail_name, modelpath="dummy-path")
