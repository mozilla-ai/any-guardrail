...
def test_list_guardrail_classes():
    with patch("any_guardrail.instantiate_guardrail._discover_guardrail_classes", return_value=None), \
...
def test_instantiate_guardrail_success():
    ...
    with patch("any_guardrail.instantiate_guardrail._discover_guardrail_classes", return_value=None), \
        ...
        obj = instantiate_guardrail.instantiate_guardrail("dummy", x=2)
...
def test_instantiate_guardrail_not_found():
    with patch("any_guardrail.instantiate_guardrail._discover_guardrail_classes", return_value=None), \
        ...
        with pytest.raises(ValueError):
            instantiate_guardrail.instantiate_guardrail("notaclass")
...
def test_instantiate_guardrail_type_error():
    ...
    with patch("any_guardrail.instantiate_guardrail._discover_guardrail_classes", return_value=None), \
         patch.dict("any_guardrail.instantiate_guardrail._GUARDRAIL_CLASSES", {"dummy": Dummy}):
        with pytest.raises(ValueError):
            instantiate_guardrail.instantiate_guardrail("dummy") 