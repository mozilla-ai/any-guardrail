...
from any_guardrail.utils.constants import (
    LABEL_UNSAFE, LABEL_SAFE, LABEL_INJECTION_LOWER,DEFAULT_THRESHOLD, DUOGUARD_CATEGORIES
)
...
class TestDuoguard:
    ...
    def test_duoguard_initialization(self):
        ...
        with patch('any_guardrail.guardrails.duoguard.AutoTokenizer') as mock_tokenizer_class, \
    ...
    def test_duoguard_classify_safe(self):
        ...
        with patch('any_guardrail.guardrails.duoguard.AutoTokenizer') as mock_tokenizer_class, \
            ...
            # Mock model output
            mock_outputs = Mock()
            mock_outputs.logits = torch.tensor([[0.1, 0.2, 0.3, 0.1, 0.2, 0.1, 0.1, 0.2, 0.1, 0.1, 0.1, 0.1]])  # All low probabilities
            mock_model.return_value = mock_outputs
            
            # Mock sigmoid
            mock_torch.sigmoid.return_value = torch.tensor([[0.1, 0.2, 0.3, 0.1, 0.2, 0.1, 0.1, 0.2, 0.1, 0.1, 0.1, 0.1]])
    ...
    def test_duoguard_classify_unsafe(self):
        ...
        with patch('any_guardrail.guardrails.duoguard.AutoTokenizer') as mock_tokenizer_class, \
            ...
            # Mock sigmoid
            mock_torch.sigmoid.return_value = torch.tensor([[0.8, 0.2, 0.3, 0.1, 0.2, 0.1, 0.1, 0.2, 0.1, 0.1, 0.1, 0.1]])
...
class TestInjecguard:
    ...
    def test_injecguard_initialization(self):
        ...
        with patch('any_guardrail.guardrails.injecguard.AutoTokenizer') as mock_tokenizer_class, \
...
class TestShieldgemma:
    ...
    def test_shieldgemma_initialization(self):
        ...
        with patch('any_guardrail.guardrails.shield_gemma.AutoTokenizer') as mock_tokenizer_class, \
...
class TestFlowjudge:
    ...
    def test_flowjudge_initialization(self):
        ...
        with patch('any_guardrail.guardrails.flowjudge.FlowJudge') as mock_flowjudge_class, \
...
class TestOtherGuardrails:
    ...
    def test_jasper_class_exists(self):
        ...
        assert hasattr(Jasper, 'model_instantiation')
        
        # Test that methods are callable
        assert callable(getattr(Jasper, 'classify'))
        assert callable(getattr(Jasper, 'model_instantiation'))
    
    def test_panoglin_class_structure(self):
        """Test that Panoglin guardrail class has proper structure."""
        from any_guardrail.guardrails.panoglin import Panoglin
        
        # Test class inheritance
        assert issubclass(Panoglin, Guardrail)
        
        # Test that class has required methods
        assert hasattr(Panoglin, 'classify')
        assert hasattr(Panoglin, 'model_instantiation')
        
        # Test that methods are callable
        assert callable(getattr(Panoglin, 'classify'))
        assert callable(getattr(Panoglin, 'model_instantiation'))
    ...
    def test_protectai_class_structure(self):
        ...
        assert hasattr(Protectai, 'model_instantiation')
        
        # Test that methods are callable
        assert callable(getattr(Protectai, 'classify'))
        assert callable(getattr(Protectai, 'model_instantiation'))
    ...
    def test_sentinel_class_structure(self):
        ...
        assert hasattr(Sentinel, 'model_instantiation')
        
        # Test that methods are callable
        assert callable(getattr(Sentinel, 'classify'))
        assert callable(getattr(Sentinel, 'model_instantiation'))
    ...
    def test_deepset_class_structure(self):
        ...
        assert hasattr(Deepset, 'model_instantiation')
        
        # Test that methods are callable
        assert callable(getattr(Deepset, 'classify'))
        assert callable(getattr(Deepset, 'model_instantiation'))
    ...
    def test_guardrail_class_signatures(self):
        ...
        guardrail_classes = [
            Jasper, Panoglin, Protectai, Sentinel, Deepset
        ]
        ...
        for guardrail_class in guardrail_classes:
            ...
            # Test that model_instantiation method exists
            assert hasattr(guardrail_class, 'model_instantiation')
...
class TestGuardrailErrorHandling:
    ...
    def test_classification_error_handling(self):
        ...
        with patch('any_guardrail.guardrails.harmguard.AutoTokenizer') as mock_tokenizer_class, \
            ...
            mock_tokenizer.return_value = {"input_ids": torch.tensor([[1, 2, 3]]), "attention_mask": torch.tensor([[1, 1, 1]])}
            mock_tokenizer.to.return_value = {"input_ids": torch.tensor([[1, 2, 3]]), "attention_mask": torch.tensor([[1, 1, 1]])}
...
class TestGuardrailIntegration:
    ...
    def test_guardrail_inheritance(self):
        ...
        guardrail_classes = [
            Glider, Harmguard, Duoguard, Injecguard, Shieldgemma, Flowjudge
        ]
    ...
    def test_guardrail_abstract_methods(self):
        ...
        for guardrail_class in guardrail_classes:
            ...
            # Check that model_instantiation method exists
            assert hasattr(guardrail_class, 'model_instantiation')
            assert callable(getattr(guardrail_class, 'model_instantiation'))
