...
| `provider` | `Optional[Provider[dict[str, Any], dict[str, Any]]]` | No | `None` | Optional execution provider whose ``infer()`` returns ``{"chunk_scores": [...]}``. ``None`` runs the local ONNX graph; pass ``ZeroDinProvider()`` for 0DIN's hosted API. |
