...
- `Guardrail` directly: API-based or fully custom shape — `AnyLlm` (any-llm SDK), `Alinia` (HTTP API), and the span / library-wrapped guardrails: `PrivacyFilter` (PII token-classification → `spans`), `LettuceDetect` (wraps the `lettucedetect` lib), `GliGuard` (wraps `gliner2`).
