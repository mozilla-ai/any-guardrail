...
# with `--check`. The registry ships empty; the benchmarks/ harness fills it out-of-repo.
...
- `schemas/guardrail_parameters.json` — generated export of the per-guardrail parameter schema (`scripts/generate_parameters_json.py`, `--check` in pre-commit), listing each guardrail's `create`/`validate` parameters (name, stage, type, required, default, choices, description) so external tooling can build config UIs without importing the package. It is **generated from signatures**: `scripts/generate_parameter_data.py` introspects every guardrail's `__init__`/`validate` and docstrings into the stdlib-only `src/any_guardrail/_parameter_data.py` leaf, which the import-free `parameter_registry.py` assembles into typed `ParameterSpec`s exposed via `AnyGuardrail.get_parameter_schema(name)`. `enum` choices are reused from `SUPPORTED_MODELS` (`model_id`), the content registry (`criteria`/`policy`/`rubric`), and the prompt registry (`prompt_version`). Parity tests in `tests/unit/test_parameters.py` enforce no drift from signatures and reconciliation with `required/optional_validate_kwargs`. Adding or changing a guardrail's params is picked up by rerunning the two generators — there is no hand-maintained parameter list.
- `schemas/guardrail_benchmarks.json` — generated export of the benchmark registry (`scripts/generate_benchmarks_json.py`, `--check` in pre-commit), and `schemas/guardrail_benchmarks.schema.json` — the JSON Schema for a `BenchmarkResult` (`scripts/generate_benchmark_schema.py`, `--check`). These back the **benchmark-backed model cards** (issue #194): each guardrail's API page renders a `## Benchmarks` table (grouped by category, `None`→`—`, provenance + contamination + comparison-cohort keys shown) and a `## License` section, from the import-free `benchmark_registry.py` + the taxonomy metadata. Benchmark numbers are hand-authored `BenchmarkResult(...)` entries in `src/any_guardrail/_benchmark_data.py`, each carrying a `BenchmarkSource` (`published:<url>` / `measured:<harness-version>`) and a `ComparisonCohort`; `group_comparable` refuses to co-rank scores across cohorts (the comparability invariant). The **registry ships empty** — the model-heavy evaluation harness lives in the unshipped top-level `benchmarks/` package (GuardBench adapter + custom-runner stubs + operational measurement; `benchmarks` dependency-group, `uv sync --group benchmarks`) and runs **out-of-repo, never in CI** (see `benchmarks/README.md`). `tests/unit/test_benchmarks.py` locks in the comparability invariant with adversarial cohort fixtures.
