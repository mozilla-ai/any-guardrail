...
# Regenerate the guardrail taxonomy metadata JSON + grouped SUMMARY nav after
# changing the registry/taxonomy (pre-commit runs both with `--check`)
python scripts/generate_metadata_json.py
python scripts/generate_summary.py

# Regenerate the machine-readable parameter schema after changing any guardrail's
# __init__/validate signature or docstring (pre-commit runs both with `--check`).
# The first introspects signatures into the stdlib-only src/any_guardrail/_parameter_data.py
# leaf; the second exports schemas/guardrail_parameters.json from the registry.
python scripts/generate_parameter_data.py
python scripts/generate_parameters_json.py

# Regenerate the benchmark JSON export + JSON Schema after editing benchmark results
# (src/any_guardrail/_benchmark_data.py) or the models (benchmarks.py). Pre-commit runs both
# with `--check`. Results are hand-authored in _benchmark_data.py; the unshipped benchmarks/ harness
# produces new numbers out-of-repo.
...
7. Add a `GUARDRAIL_METADATA` entry in `src/any_guardrail/registry.py`, and set `METADATA: ClassVar[GuardrailMetadata] = GUARDRAIL_METADATA[GuardrailName.<NAME>]` on the class next to `SUPPORTED_MODELS`. The metadata `description` **must equal the class docstring's first summary line** (a unit test enforces this), so it doubles as the guardrail's catalog flavor text. Record its `categories`/`primary_category`/`stages`/`output_shapes`/`backend`, the `required`/`optional` `validate()` kwargs, and the secondary flags. Then run `python scripts/generate_metadata_json.py` and `python scripts/generate_summary.py` and commit the results. The API-reference `GUARDRAILS` list and the grouped `docs/SUMMARY.md` navigation are now **derived** from `GuardrailName` + the registry — there are no hand-maintained lists to edit.
8. **If the guardrail authors a prompt** (generative/judge models with an in-repo template): add a `PromptSpec` entry to `PROMPT_REGISTRY` in `src/any_guardrail/prompt_registry.py` (the prompt text lives there, not in the guardrail module; record each version's `source` URL and `provenance`), set `PROMPT: ClassVar[PromptSpec] = PROMPT_REGISTRY[GuardrailName.<NAME>]` on the class next to `METADATA`, and wire the override (`prompt`/`prompt_version` constructor kwargs, or `system_prompt=`/`prompt_version=` for call-time judges like `AnyLlm`). Keep any moved module-level constant as an alias sourced from the registry so existing imports still work. Then run `python scripts/generate_prompts_json.py` and commit `schemas/guardrail_prompts.json`. Parity tests in `tests/unit/test_prompts.py` enforce the `PROMPT` ClassVar identity and the placeholder contract. (Skip for encoder classifiers and chat-template-only models that author no in-repo prompt.)
9. Tests: unit tests under `tests/unit/` (mock the provider so weights aren't downloaded); integration tests under `tests/integration/` (real model — auto-marked `e2e` by that directory's conftest; add `pytestmark = pytest.mark.heavy` if it needs >5 GB RAM/disk or a GPU so CI's default integration job skips it). The taxonomy parity tests in `tests/unit/test_metadata.py` will fail until the registry entry, the `METADATA` ClassVar, and the docstring first line all line up.
...
- `docs/SUMMARY.md` — GitBook navigation. Any new cookbook or API page must be linked from here to appear in production navigation. The **guardrail** list (between the `GUARDRAIL-NAV` markers) is generated by `scripts/generate_summary.py` from the taxonomy registry — don't hand-edit it; a `--check` pre-commit hook fails if it drifts.
- `schemas/guardrail_metadata.json` — generated export of the guardrail taxonomy registry (`scripts/generate_metadata_json.py`, `--check` in pre-commit), published like `guardrail_output.schema.json` so external tooling can query the taxonomy without importing the package.
- `schemas/guardrail_prompts.json` — generated export of the prompt registry (`scripts/generate_prompts_json.py`, `--check` in pre-commit), pinning each prompt-bearing guardrail's default/author prompt versions (with `source`/`provenance`) so external tooling and benchmarking can reference the exact prompt without importing the package.
- `schemas/guardrail_parameters.json` — generated export of the per-guardrail parameter schema (`scripts/generate_parameters_json.py`, `--check` in pre-commit). Each guardrail maps to `{"parameters": [...], "requirement_groups": [...]}` so external tooling can build config UIs without importing the package. Every `parameters` entry carries `name`, `stage`, `type`, `required`, `default`, `choices`, `description`, plus three fields the signature alone can't express: `env_var` (an environment variable that back-fills the argument, e.g. `endpoint`→`ALINIA_ENDPOINT`), `secret` (a credential to mask and never log — API keys are now surfaced with `secret=True` instead of being dropped), and `effectively_required` (a value that must exist at runtime even though the argument has a default). `requirement_groups` are guardrail-level "at least one of these interchangeable params (or their env fallback) must be provided" constraints (the `RequirementGroup` model, exposed via `AnyGuardrail.get_requirement_groups(name)`) — e.g. watsonx's `project_id`/`space_id`. It is **generated from signatures**: `scripts/generate_parameter_data.py` introspects every guardrail's `__init__`/`validate` and docstrings into the stdlib-only `src/any_guardrail/_parameter_data.py` leaf (which also embeds the requirement groups), which the import-free `parameter_registry.py` assembles into typed `ParameterSpec`s (via `AnyGuardrail.get_parameter_schema(name)`) and `RequirementGroup`s. `enum` choices are reused from `SUPPORTED_MODELS` (`model_id`), the content registry (`criteria`/`policy`/`rubric`), and the prompt registry (`prompt_version`). The runtime-requirement facts (`env_var`/`secret`/`effectively_required`/`requirement_groups`) live in hand-maintained maps in `generate_parameter_data.py` because they come from function bodies, not signatures; parity tests in `tests/unit/test_parameters.py` enforce no drift from signatures, reconciliation with `required/optional_validate_kwargs`, and — bidirectionally against the guardrail source — that every declared `env_var` is exactly what the guardrail actually reads. Adding or changing a guardrail's params is picked up by rerunning the two generators (declare any new env-var fallback / secret / group in the maps) — there is no hand-maintained parameter list.
- `schemas/guardrail_benchmarks.json` — generated export of the benchmark registry (`scripts/generate_benchmarks_json.py`, `--check` in pre-commit), and `schemas/guardrail_benchmarks.schema.json` — the JSON Schema for a `BenchmarkResult` (`scripts/generate_benchmark_schema.py`, `--check`). These back the **benchmark-backed model cards** (issue #194): each guardrail's API page renders a `## Benchmarks` table (grouped by category, `None`→`—`, provenance + contamination + comparison-cohort keys shown) and a `## License` section, from the import-free `benchmark_registry.py` + the taxonomy metadata. Benchmark numbers are hand-authored `BenchmarkResult(...)` entries in `src/any_guardrail/_benchmark_data.py`, each carrying a `BenchmarkSource` (`published:<url>` / `measured:<harness-version>`) and a `ComparisonCohort`; `group_comparable` refuses to co-rank scores across cohorts (the comparability invariant). The registry ships **hand-authored results** (published harvests plus `measured:` numbers from evaluation runs) — the model-heavy evaluation harness that produces new numbers lives in the unshipped top-level `benchmarks/` package (GuardBench adapter + custom-runner stubs + operational measurement; `benchmarks` dependency-group, `uv sync --group benchmarks`) and runs **out-of-repo, never in CI** (see `benchmarks/README.md`). `tests/unit/test_benchmarks.py` locks in the comparability invariant with adversarial cohort fixtures.
