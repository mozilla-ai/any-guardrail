...
Token-classification (span) path: when `pre_process()` is called with `return_offsets_mapping=True` (as `PrivacyFilter` does), `infer()` splits the `offset_mapping` off before the model call and returns the token-classification shape instead — `token_label_ids` / `token_scores` (per token), `offsets`, and `id2label`. `guardrails/utils.py:spans_from_token_labels()` turns those into `GuardrailOutput.spans` (greedy BIO/BIOES merge of contiguous same-entity tokens). The existing non-offset path is unchanged.
...
`GraniteGuardian`, `LlamaGuard`, and the issue-#179 decoder guardrails (`WildGuard`, `DynaGuard`, `Sguard`, `NemotronContentSafety`, `PolyGuard`, `KananaSafeguard`, `GptOssSafeguard`, `Prometheus`, `CompassJudger`, `Selene`) consume `generate_chat()` instead of touching `provider.tokenizer.apply_chat_template` + `provider.model.generate` directly, which is what lets them swap between the HF and llamafile backends. They no longer `import torch`. `chat_template_kwargs` (e.g. RAG `documents`, `available_tools`) and `generation_kwargs` (e.g. `pad_token_id` for Llama Guard 3) are pass-throughs.
...
- `Guardrail` directly: API-based or fully custom shape — `AnyLlm` (any-llm SDK), `Alinia` (HTTP API), and the span / cross-encoder / library-wrapped guardrails: `PrivacyFilter` (PII token-classification → `spans`), `Hhem` (Vectara hallucination cross-encoder, loaded directly with `trust_remote_code` since its `.predict()` API doesn't fit `pre_process`/`infer`), `LettuceDetect` (wraps the `lettucedetect` lib), `GliGuard` (wraps `gliner2`).
- `ThreeStageGuardrail` with custom generics: generative/judge models or non-binary outputs — `GraniteGuardian`, `LlamaGuard`, `Glider`, `Flowjudge`, `AzureContentSafety`, `DuoGuard` (multi-label), `OffTopic`, the decoder safety classifiers `WildGuard` / `DynaGuard` / `Sguard` / `NemotronContentSafety` / `PolyGuard` / `KananaSafeguard` / `GptOssSafeguard`, and the rubric judges `Prometheus` / `CompassJudger` / `Selene`.
...
Shared helpers live in `src/any_guardrail/guardrails/utils.py`: `default()` (model-id validation), `match_injection_label()` (the common "is the predicted label X?" post-processing), `normalize_rubric_to_risk()` (map a Likert/rubric score onto the canonical risk axis, used by the judges), and `spans_from_token_labels()` (merge per-token entity predictions + offsets into `SpanResult` character spans, used by span guardrails).
